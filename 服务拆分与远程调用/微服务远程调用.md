...
![](_v_images/20211121230226186_18701.png =666x)
服务调用关系：
       服务提供者：暴露接口给其它微服务调用
       服务消费者：调用其它微服务提供的接口
       服务者与提供者的角色是相对的
...
![](_v_images/20211126222044111_23096.png =600x)
安装Lombok
菜单栏File -> Settings -> Plugins，在中间Marketplace下面输入Lombok搜索后进行安装，安装后会提示重启IDEA。
搭建Eureka
1、 新建Moudle
![](_v_images/20211126225622291_14111.png =625x)
![](_v_images/20211126225640308_19907.png =477x)
![](_v_images/20211126225726817_17561.png =572x)
2、引入依赖
```
<dependency>
...
进入启动类，启动main()
6、访问
![](_v_images/20211126230120449_30983.png =879x)
## 服务注册
6、将服务类注册到eureka
（1）引入依赖
```
...
```
（2）编写配置文件
![](_v_images/20211128182240523_6612.png)
7、多个用户数实现场景
![](_v_images/20211128205650254_11213.png)
![](_v_images/20211128205708829_21210.png =801x)
8、实现结果
![](_v_images/20211128205532532_6951.png)
![](_v_images/20211128205550533_21050.png =1191x)
## 远程调用
调用订单服务
![](_v_images/20211128220104203_29139.png =653x)
...
```
结果：
![](_v_images/20211128221329044_10307.png =933x)
## 服务发现
我们将order-service的逻辑修改：向eureka-server拉取user-service的信息，实现服务发现。
在完成上述操作的基础上进一步进一步修改两个位置，配置加上 @LoadBalanced 注解，意味着负载均衡
![](_v_images/20211128223346491_3383.png =784x)
在服务中直接利用 userservice 代替 localhost:8081 或者 local host:8082
![](_v_images/20211128223450427_7822.png =737x)
...
![](_v_images/20211128230623661_24444.png =1369x)
...
启动成功
![](_v_images/20211130230934762_6809.png =892x)
访问：
...
```
启动项目后
![](_v_images/20211130231917947_22594.png =1526x)
### 服务分级存储模型
...
![](_v_images/20211201223039280_23714.png =1087x)
...
原因: 修改全局的编码方式为utf-8
创建集群图示
![](_v_images/20211201224827695_8202.png =943x)
...
权重值为0-1之间的
![](_v_images/20211201230445069_8346.png =708x)
### 环境隔离
nacos中可以有多个namespace
namespace下可以有group、service等
不同namespace之间相互隔离，例如不同namespace的服务互相不可见
新建一个命名空间为 dev
![](_v_images/20211201231351460_6848.png =884x)
复制 id
...
```
结果
![](_v_images/20211201231724835_7571.png =1131x)
此时访问 order 不会成功，因为服务已经隔离了
![](_v_images/20211201231956978_14095.png =1194x)
