## 概述
ReentrantLock是Java编程语言中的一个重要概念，用于多线程编程中的同步控制。它提供了与synchronized关键字类似的功能，但具有更灵活和强大的特性。

ReentrantLock的原理如下：

1. **可重入性（Reentrancy）**：ReentrantLock允许线程进入由同一个线程已经持有的锁，而不会被阻塞。这意味着一个线程可以重复地获得同一个锁，而不会发生死锁。
2. **互斥性（Mutual Exclusion）**：与synchronized关键字一样，ReentrantLock提供了互斥访问的能力，即同一时刻只允许一个线程进入临界区，防止多个线程同时访问共享资源导致的数据竞争问题。
3. **条件变量（Condition Variables）**：ReentrantLock可以与Condition接口一起使用，实现更复杂的线程通信和协作模式。它允许线程等待某个特定的条件发生，然后再继续执行。
4. **公平性（Fairness）**：ReentrantLock提供了公平性选择，可以选择是否按照线程等待锁的顺序来获取锁。公平性通常会降低性能，但可以避免饥饿情况。
5. **手动加锁和解锁**：与synchronized关键字不同，ReentrantLock需要手动加锁和解锁。这为程序员提供了更大的控制力，例如可以在不同的代码块中加锁和解锁，也可以进行更细粒度的锁定。

总的来说，ReentrantLock通过提供灵活性、可重入性、条件变量等特性，为多线程编程提供了更高级别的控制和功能，但也需要开发人员更多地关注锁的获取和释放，以避免死锁等问题。
...
![image](https://pdai.tech/images/thread/java-thread-x-juc-reentrantlock-2.png)
...
![image](https://pdai.tech/images/thread/java-thread-x-juc-reentrantlock-3.png)
...
![image](https://pdai.tech/images/thread/java-thread-x-juc-reentrantlock-4.png)
...
![image](https://pdai.tech/images/thread/java-thread-x-juc-reentrantlock-5.png)

说明: 由调用流程可知，t1线程成功获取了资源，可以继续执行。

- t2线程执行lock.lock，下图给出了方法调用中的主要方法。

![image](https://pdai.tech/images/thread/java-thread-x-juc-reentrantlock-6.png)

说明: 由上图可知，最后的结果是t2线程会被禁止，因为调用了LockSupport.park。

- t3线程执行lock.lock，下图给出了方法调用中的主要方法。

![image](https://pdai.tech/images/thread/java-thread-x-juc-reentrantlock-7.png)

说明: 由上图可知，最后的结果是t3线程会被禁止，因为调用了LockSupport.park。

- t1线程调用了lock.unlock，下图给出了方法调用中的主要方法。

![image](https://pdai.tech/images/thread/java-thread-x-juc-reentrantlock-8.png)

说明: 如上图所示，最后，head的状态会变为0，t2线程会被unpark，即t2线程可以继续运行。此时t3线程还是被禁止。

- t2获得cpu资源，继续运行，由于t2之前被park了，现在需要恢复之前的状态，下图给出了方法调用中的主要方法。

![image](https://pdai.tech/images/thread/java-thread-x-juc-reentrantlock-9.png)
...
![image](https://pdai.tech/images/thread/java-thread-x-juc-reentrantlock-10.png)

说明: 由上图可知，最终unpark t3线程，让t3线程可以继续运行。

- t3线程获取cpu资源，恢复之前的状态，继续运行。

![image](https://pdai.tech/images/thread/java-thread-x-juc-reentrantlock-11.png)
